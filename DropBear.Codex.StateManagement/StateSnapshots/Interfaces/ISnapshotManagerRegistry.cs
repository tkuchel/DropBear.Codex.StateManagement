...
public interface ISnapshotManagerRegistry
{
    ...
    Result<T> GetManager<T>(string key) where T : ICloneable<T>;
    ...
    void Register<T>(StateSnapshotManager<T> manager, string key, bool overwrite = false) where T : ICloneable<T>;
